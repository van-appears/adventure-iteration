from game.Container import Container
from game.state import inventory

class Location(Container):
    def __init__(self, items=dict()):
        self.visited = False
        super().__init__(items);

    def visit(self):
        self.visited = True

    def was_visited(self):
        return self.visited

    def short_description(self):
        pass

    def long_description(self):
        short_description()

    def perform_action(self, action):
        pass

    def print_items(self):
        keys = self.item_keys()
        if len(keys) > 0:
            print(f"Here there is: {", ".join(self.item_keys())}")

    def take_item(self, noun_key):
        if inventory().has_item(noun_key):
            print(f"You already have: {noun_key}!")
        else:
            done = False
            for location_key in self.item_keys():
                if noun_key == location_key:
                    inventory().add_item(noun_key)
                    self.remove_item(noun_key)
                    print(f"You have taken: {noun_key}")
                    done = True
            if not done:
                print("There isn't one of these here.")
                self.print_items()
