...
def reset_state():
    ...
    STATE["combination"] = f"{day}{month}"
    STATE["birthday"] = f"{day} {MONTHS[month]}"
    STATE["book_name"] = random.choice(list(BOOKS.keys()))
    STATE["book_key"] = STATE["book_name"].lower()
    STATE["author"] = BOOKS[STATE["book_name"]]
