import unittest
from game import main
from base.state import reset_state, set_state, has_state
from base.map import reset_map

class TestStringMethods(unittest.TestCase):
    def setUp(self):
        print("\n====================")
        print(self._testMethodName)
        print("====================")
        set_state("__GET_ACTION", None)
        reset_state()
        reset_map()

    def create_actions(self, script):
        commands = iter(script.split("\n"))
        def actions(_=None):
            command = next(commands).strip()
            print("> TESTING:", command)
            return command
        set_state("__GET_ACTION", actions)

    def test_win_no_fire(self):
        combination = has_state("combination")
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            examine photo
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            open chest
            {combination}
            take key
            go east
            light candle
            take {book_key}
            go west
            read {book_key}
            take key
            go north
            go north
            open cupboard
            take honey
            go south
            give honey
            open door
        """)

        main()
        self.assertTrue(has_state("escaped"))
        self.assertFalse(has_state("died"))

    def test_win_fire(self):
        combination = has_state("combination")
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            examine photo
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            open chest
            {combination}
            take key
            go east
            light candle
            read {book_key}
            take key
            go to kitchen
            open cupboard
            take honey
            go south
            give honey
            open door
        """)

        main()
        self.assertTrue(has_state("escaped"))
        self.assertFalse(has_state("died"))

    def test_near_win(self):
        combination = has_state("combination")
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            examine photo
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            open chest
            {combination}
            take key
            go east
            light candle
            read {book_key}
            take key
            go west
            go north
            go north
            open cupboard
            take honey
            go south
            give honey
            open door
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_bear_death_1(self):
        actions = self.create_actions("""
            attack bear
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_bear_death_2(self):
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            go east
            light candle
            read {book_key}
            take key
            go west
            go north
            go north
            open cupboard
            take hotsauce
            go south
            give hotsauce
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_bear_death_3(self):
        actions = self.create_actions("""
            go north
            open drawers
            take hammer
            go south
            throw hammer
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_match_death_1(self):
        actions = self.create_actions("""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            light match
            light match
            light match
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_match_death_2(self):
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            go east
            light candle
            read {book_key}
            take key
            read {book_key}
            read {book_key}
            read {book_key}
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_match_death_3(self):
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            go east
            light candle
            read {book_key}
            take key
            go west
            describe chest
            describe chest
            describe chest
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

    def test_match_death_4(self):
        book_key = has_state("book_key")
        actions = self.create_actions(f"""
            go north
            open drawers
            take hammer
            smash bottle
            take matches
            go south
            go east
            go north
            read notebook
            go south
            go south
            take candle
            go north
            go east
            go south
            go east
            light candle
            read {book_key}
            take key
            go west
            go north
            go east
            go east
            go east
            go east
            go east
            go east
            go east
        """)

        main()
        self.assertFalse(has_state("escaped"))
        self.assertTrue(has_state("died"))

if __name__ == '__main__':
    unittest.main()
