...
from things.earthsea import Earthsea
...
class Library(Location):
    ...
    def perform_action(self, action):
        ...
        if verb == "read":
            ...
            if noun == "earthsea":
                self.ensure_earthsea_present(True)
                if not inventory().has_direct_item_with_key("earthsea"):
    ...
    def take_item(self, noun_key):
        if noun_key == "earthsea" and not inventory().has_direct_item_with_key("earthsea") and not self.has_direct_item_with_key("earthsea") and has_state("candle_lit"):
            print("You find the copy of Earthsea on the shelves.")
            self.ensure_earthsea_present()
    ...
    def ensure_earthsea_present(self, include_key=False):
        if not inventory().has_direct_item_with_key("earthsea") and not self.has_direct_item_with_key("earthsea"):
            earthsea = Earthsea()
            self.add_item(earthsea)
            if include_key:
                earthsea.add_item(KitchenCupboardKey())
