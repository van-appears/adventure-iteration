...
class KitchenCupboard(Thing):
    def __init__(self):
        super().__init__(keys=["cupboard", "cupboards"])
        self.opened = False
    ...
    def perform_action(self, action):
        ...
        if verb == "open":
            ...
            elif "small key" in inventory().item_keys():
                print("You unlock the cupboard with the small key.")
                print("There is a bottle of hotsauce and a pot of honey inside.")
                self.items.extend([Honey(), Hotsauce()])
                self.opened = True
            elif "door key" in inventory().item_keys():
                print("You only have a door key, which does not work here.")
            else:
